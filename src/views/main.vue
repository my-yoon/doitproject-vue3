    ...
    <section class="maincontents">
        ...
        <section class="newbook">
            ...
            <div class="tapcontent">
                <div class="tap">
                    <button type="button" class="btn-tap active" @click="AddContents('Newbooks_it')">IT 전문서</button>
                    <button type="button" class="btn-tap" @click="AddContents('Newbooks_work')">업무 능력, 기타</button>
                    <button type="button" class="btn-tap" @click="AddContents('Newbooks_photo')">사진 예술</button>
                    <button type="button" class="btn-tap" @click="AddContents('Newbooks_science')">데이터 과학</button>
                </div>
                <Carousel class="mainslider" :breakpoints="state.breakpoints">
        ...
        <section class="searchbook">
            ...
            <div class="inputarea">
                ...
                <div class="autolayer" v-if="state.keyword.length > 0"  :class="{ none: autoComplate.length <= 0 }" >
            ...
            <div class="guidehash">
                <span
                v-for="(item, index) in state.hashdata"
                :key="index"
                v-html="item.text"
                ></span>
            </div>
        ...
        <section class="bannermenu">
            <div class="itembox" v-for="(item, index) in state.mainbanner" :key="index" @click="gotoPage(item.url)">
        ...
        <section class="notice">
            <div class="noticebox">
                ...
                <button type="button" class="btn-noticemore" @click="viewMore($event)"></button>
                <div class="newsitem" v-for="(item, index) in state.noticelists" :key="index">
...
const AddContents = (contents) => {
    state.Newbooks = state[contents];
    document.querySelector('.active').classList.remove('active');
    event.target.classList.add('active');
};
onMounted(async () => {
    state.Newbooks = state.Newbooks_it;
});
const autoComplate = computed(() => {
    const resultlists = state.booksname.filter((item) => {
        if (item.cata.match(state.keyword))
            return item;
    });
    return resultlists;
});
